function [string, terminatorChar] = echoString(windowPtr, msg, x, y, textColor, bgColor, is_disable_alphabet, useKbCheck)
...
Screen('TextFont', windowPtr, 'Courier');
DrawFormattedText(windowPtr, output, 'Center', 'Center', textColor, bgColor)
% Screen('DrawFormattedText', windowPtr, msg, x, y, textColor, bgColor);
Screen('Flip', windowPtr, 0, 1);

alphabe_str = char(97:122);
alphabe_cell = cellstr(alphabe_str(:));
numpad_str = '0123456789';
while true
    char_str = GetChar;
    if isempty(char_str)
        string = '';
    ...
    switch abs(char_str)
        ...
        case 8
            ...
            if ~isempty(string)
                ...
                % Remove last char_stracter from string:
                string = string(1:length(string)-1);
            end
        case alphabe_cell
            if is_disable_alphabet
                continue
            else
                string = [string, char_str];
            end
        otherwise
            if length(string) >1
                continue
            end
            if ~strfind(numpad_str, char_str)
                continue
            end
            string = [string, char_str]; %#ok<AGROW>
    end
    output = [msg, '\n', string];
    DrawFormattedText(windowPtr, double(output), 'Center', 'Center', textColor, bgColor)
    Screen('Flip', windowPtr, 0, 1);
end

% Restore text12
% alpha blending state if it was altered:
% if ~isempty(bgColor)
